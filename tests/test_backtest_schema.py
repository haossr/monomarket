...
def test_validate_nightly_summary_sidecar_reject_bad_reject_by_strategy_top_reason_mismatch() -> (
    None
):
    payload = _nightly_sidecar_payload()
    reject_by_strategy = payload["reject_by_strategy"]
    assert isinstance(reject_by_strategy, dict)
    reject_by_strategy["top_reason"] = "different"

    with pytest.raises(ValueError):
        validate_nightly_summary_sidecar(payload)


def test_validate_nightly_summary_sidecar_reject_bad_reject_by_strategy_top_rejected_mismatch() -> (
    None
):
    payload = _nightly_sidecar_payload()
    reject_by_strategy = payload["reject_by_strategy"]
...
def test_validate_nightly_summary_sidecar_reject_bad_reject_by_strategy_non_top_rejected_mismatch() -> (
    None
):
    payload = _nightly_sidecar_payload()
    reject_by_strategy = payload["reject_by_strategy"]
    assert isinstance(reject_by_strategy, dict)
    reject_by_strategy["non_top_rejected"] = 0
...
def test_validate_nightly_summary_sidecar_reject_bad_negative_event_zero_case() -> None:
    payload = _nightly_sidecar_payload()
    negative_events = payload["negative_events"]
    assert isinstance(negative_events, dict)
    negative_events["count"] = 0
    negative_events["worst_event_id"] = "12345"
    negative_events["worst_strategy"] = "s2"
    negative_events["worst_pnl"] = -0.4

    with pytest.raises(ValueError):
        validate_nightly_summary_sidecar(payload)


def test_validate_nightly_summary_sidecar_reject_bad_negative_event_positive_case() -> None:
    payload = _nightly_sidecar_payload()
    negative_events = payload["negative_events"]
    assert isinstance(negative_events, dict)
...
def test_validate_nightly_summary_sidecar_reject_bad_negative_event_source_missing_with_positive_count() -> (
    None
):
    payload = _nightly_sidecar_payload()
    negative_events = payload["negative_events"]
    assert isinstance(negative_events, dict)
    negative_events["source_present"] = False

    with pytest.raises(ValueError):
        validate_nightly_summary_sidecar(payload)


def test_validate_nightly_summary_sidecar_reject_bad_rolling_negative_zero_case() -> None:
    payload = _nightly_sidecar_payload()
    rolling = payload["rolling"]
    assert isinstance(rolling, dict)
    rolling_negative = rolling["negative_strategies"]
    assert isinstance(rolling_negative, dict)
    rolling_negative["count"] = 0
    rolling_negative["worst_strategy"] = "s2"
    rolling_negative["worst_avg_pnl"] = -0.2

    with pytest.raises(ValueError):
        validate_nightly_summary_sidecar(payload)


def test_validate_nightly_summary_sidecar_reject_bad_rolling_negative_positive_case() -> None:
    payload = _nightly_sidecar_payload()
    rolling = payload["rolling"]
    assert isinstance(rolling, dict)
    rolling_negative = rolling["negative_strategies"]

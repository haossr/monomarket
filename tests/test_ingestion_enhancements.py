...
def test_cli_ingest_health(tmp_path: Path) -> None:
    ...
    shares_min_count2 = storage.list_ingestion_error_bucket_share_by_source(
        source="gamma",
        run_window=5,
        min_bucket_count=2,
    )
    assert len(shares_min_count2) == 1
    assert str(shares_min_count2[0]["error_bucket"]) == "http_5xx"

    shares_min_runs2 = storage.list_ingestion_error_bucket_share_by_source(
        source="gamma",
        run_window=5,
        min_runs_with_error=2,
    )
    ...
    shares_min_source_total5 = storage.list_ingestion_error_bucket_share_by_source(
        source="gamma",
        run_window=5,
        min_source_bucket_total=5,
    )
    assert shares_min_source_total5 == []
    ...
    res = runner.invoke(
        ...
        [
            ...
            "2",
            "--error-share-min-source-bucket-total",
            "4",
            "--error-sample-limit",
            "3",
            "--config",
    ...
    assert "min_source_bucket_total=4" in res.output

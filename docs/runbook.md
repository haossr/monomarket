...
>
> PDF 会展示收益图表，同时在 Sample Coverage 区块展示总览胜率（Closed/MTM winrate summary，来源与 nightly summary 一致：`latest.json.results`）与主窗口样本覆盖注记（Main window coverage / Main history limited / Main window note）；并在 Rolling Summary 区块展示 rolling execution rate / empty windows / coverage label 与 rolling reject top 摘要（简版 `reason:count;reason:count`）。

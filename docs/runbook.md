...
- `summary.json`（结构化 sidecar，便于机器解析；含 `best` 对象、`best_version`、`schema_note_version`、`schema_note`、rolling 指标与 `checksum_algo/checksum_sha256`；当 `executed_signals=0` 时 `best.available=false` 且 `best_strategy=n/a`）

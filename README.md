...
报告会自动包含收益图表（累计 realized PnL 曲线 + 策略 PnL 柱状图；数据不足时显示降级提示），并在 Sample Coverage 区块展示总览胜率（Closed/MTM winrate summary，来源与 nightly summary 一致：`latest.json.results`）与主窗口样本覆盖注记（Main window coverage / Main history limited / Main window note）；当 `history_limited=true` 时会额外给出谨慎解读提示。`Sx12 Strategy Focus / S9-S10` 卡片展示 S9/S10 的 present/active、PnL、trades、closed/mtm winrate，以及 activity_hint/replay_rows/rejected_rows/top_reject_reason；并同步展示 `top_reject_reason_source` 与 `generation_rejected_candidates/generation_top_reject_reason`，以及 tiny/floor 相关 reject count+share（`generation_tiny_price_leg_*`、`generation_floor_adjusted_leg_*`，无 replay 时回退到 signal-generation 诊断）。若传入 `--sx12-compare-json`，同卡片还会渲染 Guard snapshot（`s9_cross_market_require_same_source` / `s10_require_same_source` 的 baseline/candidate）。若同时传入 `--s10-grid-json`，同卡片还会追加 S10 same-source rollup（`s10_grid_same_source_false/true`：候选数、通过率、均值 `ΣΔpnl`）；nightly summary/sidecar 会同步附带 `same_source_rollup`（`require_same_source=true/false` 两桶）。Rolling Summary 区块展示 rolling execution rate / empty windows / coverage label 与 rolling reject top 摘要（简版 `reason:count;reason:count`）。
...
若需要在每个切片对 baseline/candidate 先重建信号（隔离 DB，避免污染主库），可开启：
...
- 结算口径门控默认开启：若某候选在任一切片出现 `settle_mismatch_slice_count>0`，会被标记 `pass_settle?=no` 并降级为 `pass=false`。若需仅观测（不拦截），可加 `--allow-settle-profile-mismatch`。QA 回归可用 `--inject-candidate-settle-mismatch` 强制候选关闭 settle 以验证门控。

...
def ingest_health(
    ...
    error_share_min_count: int = typer.Option(
        0,
        min=0,
        max=100000,
        help="Drop error-share buckets with count below this threshold (default 0=all)",
    ),
    error_share_min_runs_with_error: int = typer.Option(
        0,
        min=0,
        max=100000,
    ...
    error_share_min_source_bucket_total: int = typer.Option(
        0,
        min=0,
        max=100000,
        help="Drop sources with aggregate bucket_total below this threshold in error share table",
    ),
    ...
    if isinstance(edge_gate, dict):
        ...
        if isinstance(by_strategy_diag, dict) and by_strategy_diag:
            ...
            if strategy_reject_rows:
                ...
                reject_tb.add_column("strategy")
                reject_tb.add_column("reason")
                reject_tb.add_column("count")
